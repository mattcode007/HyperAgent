# dashboard.py
import streamlit as st, pandas as pd, plotly.graph_objects as go, json, os, config, database, time, numpy as np
...
def display_dashboard():
    ...
    def load_trade_data():
        ...
        except pd.io.sql.DatabaseError: trades_df = pd.DataFrame()
        conn.close()
        return trades_df
    ...
    with tab2:
        ...
        else: st.info("No trades to display.")

    with tab3:
        st.header("Model Training Performance")
        if os.path.exists(config.TRAINING_HISTORY_FILE):
            with open(config.TRAINING_HISTORY_FILE, 'r') as f: history = json.load(f)
            history_df = pd.DataFrame(history)
            fig_train = go.Figure(go.Scatter(x=history_df['episode'], y=history_df['reward'], mode='lines+markers'))
            fig_train.update_layout(title="Agent's Learning Curve (Reward per Episode)", template="plotly_dark")
